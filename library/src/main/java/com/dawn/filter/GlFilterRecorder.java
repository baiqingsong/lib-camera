...
class GlFilterRecorder {
    ...
    private void releaseEgl() {
        if (eglDisplay != EGL14.EGL_NO_DISPLAY) {
            ...
            // 注意：绝不能调用 eglTerminate()！
            // EGLDisplay 是进程级共享的——相机预览的 GPUImageView 也在使用同一个
            // 默认 display。eglTerminate 会销毁整个 display 上的所有 GL 资源，
            // 导致预览的 shader program / 纹理失效，录制结束后返回预览黑屏
            // （Mali 报 "program is not a value generated by OpenGL"）。
            // 这里只销毁录制器自己创建的 surface 和 context 即可。
        }
